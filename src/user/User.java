...
public class User {
    ...
    public void Inscription(BDD BDD) throws SQLException {
        PreparedStatement req = BDD.getCnx().prepareStatement("INSERT INTO compte(nom, prenom, email, mdp) VALUES (?,?,?,?)");
        req.setString(1,nom);
    ...
    public boolean Connexion(BDD BDD) throws SQLException {
        PreparedStatement req = BDD.getCnx().prepareStatement("SELECT * FROM compte WHERE email = ? AND mdp = ?");
    ...
    public void Modification(BDD BDD) throws SQLException {
        PreparedStatement req = BDD.getCnx().prepareStatement("UPDATE compte SET nom = ?, prenom = ?, email = ?, mdp = ? WHERE id_compte = ?");
        req.setString(1,nom);
        req.setString(2,prenom);
        req.setString(3,email);
        req.setString(4,mdp);
    ...
    public void Suppression(BDD BDD) throws SQLException {
        PreparedStatement req = BDD.getCnx().prepareStatement("DELETE FROM compte WHERE id_compte = ?");
